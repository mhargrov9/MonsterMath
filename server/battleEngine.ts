...
// This now correctly includes hp and mp, which are added at runtime.
...
export async function processBattleAction(initialState: BattleState, action: Action) {
    ...
    if (action.type === 'USE_ABILITY') {
        ...
        if ((attacker.hp ?? 0) > 0 && ability) {
            ...
            aiTeam = aiTeam.map((m: AiCombatMonster, i: number) => i === activeAiIndex ? { ...m, hp: Math.max(0, m.hp - damage) } : m);
            playerTeam = playerTeam.map((m: PlayerCombatMonster, i: number) => i === activePlayerIndex ? { ...m, mp: (m.mp ?? 0) - (ability.mp_cost || 0) } : m);
        ...
        if (newIndex !== -1 && (playerTeam[newIndex].hp ?? 0) > 0) {
    ...
    if (activeAi.hp > 0) {
        ...
        if ((playerDefender.hp ?? 0) > 0) {
            ...
            if (aiAbility) {
                ...
                aiTeam = aiTeam.map((m: AiCombatMonster, i: number) => i === activeAiIndex ? { ...m, mp: m.mp - (aiAbility.mp_cost || 0)} : m);
