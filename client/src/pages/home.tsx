  ...
  const addRepairKitMutation = useMutation({
    mutationFn: async () => {
      await apiRequest('POST', '/api/inventory/add', {
        itemName: 'Repair Kit',
        itemDescription:
          'Repairs a shattered monster back to full health. Essential for monster trainers!',
        quantity: 1,
        itemType: 'consumable',
        rarity: 'rare',
  ...
  return (
    <div className="min-h-screen">
          ...
          <div className="max-w-7xl mx-auto">
            <div className="hidden lg:flex justify-between items-center">
              <div className="flex items-center space-x-4">
                <h1 className="text-3xl font-fredoka text-white">
                  Monster Academy
                </h1>
              </div>
      ...
      <div
        className={
          activeTab !== 'battle'
            ? 'max-w-7xl mx-auto p-3 sm:p-4 lg:p-6'
            : 'w-full h-full'
        }
      >
          ...
          <div className="mb-6 sm:mb-8">
            <div className="flex space-x-4 bg-white/10 p-2 rounded-2xl backdrop-blur-sm">
              <Button
                ...
                className={`flex-1 px-6 py-4 rounded-xl font-bold text-lg transition-all ${activeTab === 'lab' ? 'bg-electric-blue text-white animate-pulse-glow' : 'bg-white/20 text-white hover:bg-white/30'}`}
              >
                MONSTER LAB
              </Button>
              <Button
                onClick={() => setActiveTab('battle')}
                className={`flex-1 px-6 py-4 rounded-xl font-bold text-lg transition-all ${activeTab === 'battle' ? 'bg-electric-blue text-white animate-pulse-glow' : 'bg-white/20 text-white hover:bg-white/30'}`}
              >
                BATTLE ARENA
              </Button>
              <Button
                onClick={() => setActiveTab('story')}
                className={`flex-1 px-6 py-4 rounded-xl font-bold text-lg transition-all ${activeTab === 'story' ? 'bg-electric-blue text-white animate-pulse-glow' : 'bg-white/20 text-white hover:bg-white/30'}`}
              >
                STORY
              </Button>
            </div>
