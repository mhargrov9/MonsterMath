...
const MonsterGraphics: React.FC<MonsterGraphicsProps> = ({
  ...
  const renderMonsterContent = () => {
    const hasSharpTeeth =
      upgradeChoices.teeth === 'sharp' ||
      upgradeChoices.teeth === 'icy' ||
      upgradeChoices.teeth === 'electric';
    const hasTail =
      upgradeChoices.tail === 'spiky' ||
      upgradeChoices.tail === 'crystalline' ||
      upgradeChoices.tail === 'lightning';
    const isMuscular =
      upgradeChoices.body === 'muscular' ||
      upgradeChoices.body === 'armored' ||
      upgradeChoices.body === 'charged';
    const hasWings = upgradeChoices.wings === 'large';
    const hasSpikes =
      upgradeChoices.spikes === 'ice' || upgradeChoices.spikes === 'electric';
        ...
        {(monsterId === 1 || evolutionStage >= 2) && (
          <>
            <path
              d={
                hasWings
                  ? 'M65 90 Q40 70 45 110 Q60 100 75 105'
                  : 'M70 90 Q55 80 60 105 Q70 100 75 100'
              }
            ...
            <path
              d={
                hasWings
                  ? 'M135 90 Q160 70 155 110 Q140 100 125 105'
                  : 'M130 90 Q145 80 140 105 Q130 100 125 100'
              }
              fill={`url(#wingGradient-${monsterId})`}
              opacity="0.8"
            />
          ...
          <>
            ...
            {monsterId === 2 && (
              <>
                ...
                />
                <polygon
                  points="170,120 175,115 180,125 175,130"
                  fill="#a8e6cf"
                />
              </>
            )}
          ...
          <>
            ...
            {monsterId === 3 && (
              <>
                ...
                <path
                  d="M115 90 L120 85 L112 88 L117 80 L110 85"
                  fill="#f1c40f"
                />
              </>
            )}
      ...
      >
        <defs>
          ...
          <radialGradient id={`aura-${monsterId}`}>
            <stop
              offset="0%"
              stopColor={getMonsterColors().accent}
              stopOpacity="0"
            />
            <stop
              offset="70%"
              stopColor={getMonsterColors().primary}
              stopOpacity="0.2"
            />
            <stop
              offset="100%"
              stopColor={getMonsterColors().secondary}
              stopOpacity="0.4"
            />
          </radialGradient>
          <linearGradient
            id={`specialEffect-${monsterId}`}
            x1="0%"
            y1="0%"
            x2="100%"
            y2="0%"
