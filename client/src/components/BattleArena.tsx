          ...
          {!canBattle ? (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              ...
              <p className="text-yellow-700 mb-3">
                Answer questions correctly to earn battle tokens! You get 1 token for every 1 correct answer.
              </p>
              <div className="bg-yellow-100 rounded-lg p-3">
                <div className="flex justify-between text-sm text-yellow-800 mb-1">
                  <span>Progress to next token:</span>
                  <span>{nextTokenProgress}/1</span>
                </div>
                <div className="w-full bg-yellow-200 rounded-full h-2">
                  <div 
                    className="bg-yellow-500 h-2 rounded-full transition-all" 
                    style={{ width: `${(nextTokenProgress / 1) * 100}%` }}
                  ></div>
                </div>
                <p className="text-xs text-yellow-600 mt-2">
                  {1 - nextTokenProgress} more correct answers needed
                </p>
              </div>
            </div>
          ) : (
