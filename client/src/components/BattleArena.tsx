...
export default function BattleArena({ onRetreat }: BattleArenaProps) {
  ...
  const handleTargetSelected = async (targetId: number) => {
    if (!targetingState.isTargeting || !targetingState.ability || !battleId)
      return;
    ...
    try {
      ...
      const targetMonster =
        playerTeam.find((m) => m.id === targetId) ||
        aiTeam.find((m) => m.id === targetId);
      if (targetMonster) {
        addFloatingText(
          isHealing
            ? `+${Math.abs(damageResult.damage)}`
            : `-${damageResult.damage}`,
      ...
      setBattleLog((prev) => [
        ...prev,
        'Error performing targeted action! Please try again.',
      ]);
    } finally {
  ...
  const handleSwapMonster = async (monsterId: number) => {
    ...
    try {
      ...
      setBattleLog((prev) => [
        ...prev,
        'Error swapping monster! Please try again.',
      ]);
    }
  };
        ...
        {turn === 'player-must-swap' && (
          <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 bg-red-600 text-white px-6 py-3 rounded-lg shadow-lg border-2 border-red-400 animate-pulse">
            <div className="text-center">
              ...
              <p className="text-sm">
                Choose a new monster from your bench to continue.
              </p>
            </div>
          </div>
        )}
