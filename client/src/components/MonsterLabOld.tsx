...
export default function MonsterLab() {
  ...
  const { data: monsters = [], isLoading: monstersLoading } = useQuery<
    Monster[]
  >({
    queryKey: ['/api/monsters'],
  });

  const { data: userMonsters = [], isLoading: userMonstersLoading } = useQuery<
    UserMonster[]
  >({
  ...
  const purchaseMutation = useMutation({
    mutationFn: async (monsterId: number) => {
      const response = await apiRequest('POST', '/api/monsters/purchase', {
        monsterId,
      });
  ...
  const upgradeMutation = useMutation({
    mutationFn: async (userMonsterId: number) => {
      const response = await apiRequest('POST', '/api/monsters/upgrade', {
        userMonsterId,
      });
      return response.json();
    },
    ...
    return (
      <div className="space-y-6">
        <div className="text-center mb-8">
          <h2 className="font-fredoka text-4xl text-white mb-2">
            🧪 Monster Laboratory 🧪
          </h2>
    ...
    <div className="space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-white/80 text-lg">
          Buy, upgrade, and manage your monster collection!
        </p>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {userMonsters.map((userMonster) => (
            <Card
              key={userMonster.id}
              className="bg-white rounded-2xl shadow-xl border-4 border-electric-blue transform hover:scale-105 transition-transform"
            >
              <CardContent className="p-6">
                {/* Monster illustration */}
                <div
                  className={`w-full h-32 bg-gradient-to-br ${userMonster.monster.gradient} rounded-xl mb-4 flex items-center justify-center`}
                >
                  <i
                    className={`${userMonster.monster.iconClass} text-4xl text-white`}
                  ></i>
                </div>
          ...
          {monsters.map((monster) => (
            ...
            >
              <CardContent className="p-4">
                {/* Monster illustration */}
                <div
                  className={`w-full h-24 bg-gradient-to-br ${monster.gradient} rounded-lg mb-3 flex items-center justify-center`}
                >
                ...
                <div className="flex justify-between items-center">
                  <div className="flex flex-col">
                    <span className="text-gold-yellow font-bold">
                      {monster.goldCost} Gold
                    </span>
                    {monster.diamondCost > 0 && (
                      <span className="text-diamond-blue font-bold">
                        {monster.diamondCost} Diamonds
                      </span>
                    )}
                  </div>
                  <Button
